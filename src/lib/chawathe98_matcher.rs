...
pub enum EdgeType {
    /// Check the type of edge update which is update
    UPDATE,
    /// Check the type of edge update which is insert
    INSERT,
    /// Check the type of edge update which is delete
    DELETE,
    /// Check the type of edge update which is move
    MOVE,
    /// Check the type of edge update which is copy
    COPY,
    /// Check the type of edge update which is glue
    GLUE,
    /// Check the type of edge update which is null
    NULL,
    /// Check the type of edge update which is Ok
    OK,
}
#[derive(Debug, Clone)]
/// The intial cost of each edge
pub struct CostEdge {
    /// Check the cost of edge update which is update
    pub cost_update: usize,
    /// Check the cost of edge update which is insert
    pub cost_insert: usize,
    /// Check the cost of edge update which is delete
    pub cost_delete: usize,
    /// Check the cost of edge update which is move
    pub cost_move: usize,
    /// Check the cost of edge update which is copy
    pub cost_copy: usize,
    /// Check the cost of edge update which is glue
    pub cost_glue: usize,
    /// Check the cost of edge update which is null
    pub cost_null: usize,
    /// Check the cost of edge update which is Ok
    pub cost_ok: usize,
}

/// implemenation for edge cost
...
impl CostEdge {
    ...
    pub fn new(
        ...
        CostEdge {
            cost_update: cost_update_1,
            cost_insert: cost_insert_1,
            cost_delete: cost_delete_1,
            cost_move: cost_move_1,
            cost_copy: cost_copy_1,
            cost_glue: cost_glue_1,
            cost_null: cost_null_1,
            cost_ok: cost_ok_1,
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct MappingStore2<T: Clone + Debug> {
    ///List Of edges
    pub list_edges: Vec<Edge>,
    ...
    pub list_node_inserted: Vec<NodeId<ToNodeId>>,
    /// List of nodes to be deleted
    pub list_node_deleted: Vec<NodeId<FromNodeId>>,
    /// Find the Cost of all edges
    pub all_edge_cost: CostEdge,
}

impl<T: Clone + Debug + Eq + 'static> MappingStore2<T> {
    /// Implementation of new in mapping store the second version
    pub fn new(base: Arena<T, FromNodeId>, diff: Arena<T, ToNodeId>) -> MappingStore2<T> {
        MappingStore2 {
            from_arena: base,
            to_arena: diff,
            list_edges: Vec::new(),
            list_node_inserted: Vec::new(),
            list_node_deleted: Vec::new(),
    ...
    pub fn push(
        ...
        self.list_edges.push(new_edge);
    }
    /// Remove Edge
    pub fn remove(&mut self, index: usize) {
        self.list_edges.remove(index);
    }
    ...
    pub fn print_preorder_traversal(
        ...
        print!("|");
        for node in tree1 {
            print!("{:?},{:?}|", node.id(), self.from_arena[node].label);
        }
        println!();
        println!("Tree 2");
        print!("|");
        for node in tree2 {
            print!("{:?},{:?}|", node.id(), self.to_arena[node].label);
        }
        println!();
    }
    ...
    pub fn check_subtree(&self, from_node: NodeId<FromNodeId>, to_node: NodeId<ToNodeId>) -> bool {
        ...
        let descendants_from_node = from_node
            .descendants(&self.from_arena)
            .collect::<Vec<NodeId<FromNodeId>>>();
        let descendants_to_node = to_node
            .descendants(&self.to_arena)
            .collect::<Vec<NodeId<ToNodeId>>>();
        if eq(&from_node, &self.from_arena, &to_node, &self.to_arena) {
    ...
    pub fn lower_bound_edge(self, edge1: Edge, edges: Vec<Edge>) -> usize {
        ...
        let cost_update = self.all_edge_cost.cost_update;
        let cost_move = self.all_edge_cost.cost_move;
        let m = edge1.from_node.clone();
        let n = edge1.to_node.clone();

        // Sum of Cost of forced move with m',n where m' is the child of m
        let m_children = m.children(&self.from_arena)
            .collect::<Vec<NodeId<FromNodeId>>>();
        let n_children = n.children(&self.to_arena)
            .collect::<Vec<NodeId<ToNodeId>>>();
    ...
    pub fn upper_bound_edge(self, edge1: Edge, edges: Vec<Edge>) -> usize {
        ...
        for m_child in m_children.clone() {
            ...
            //
            cm_m1_n = self.all_edge_cost.cost_copy + cm_m1_n + em1;
        }
        ...
        for n_child in n_children.clone() {
            ...
            //
            cm_m_n1 = self.all_edge_cost.cost_glue + cm_m_n1 + en1;
        }

        let mut cost_cw = 0;

        if edge1.edge_type == EdgeType::UPDATE {
            cost_cw = self.all_edge_cost.cost_update;
        } else if edge1.edge_type == EdgeType::INSERT {
            cost_cw = self.all_edge_cost.cost_insert;
        } else if edge1.edge_type == EdgeType::DELETE {
            cost_cw = self.all_edge_cost.cost_delete;
        }
    ...
    pub fn annotate(&self, edges: Vec<Edge>) {
        ...
        for edge in edges.clone() {
            ...
            if list_m.len() == 1 && list_n.len() == 1 {
                ...
                if list_m[0].id() == self.from_arena.size() - 1
                    && list_n[0].id() != self.to_arena.size() - 1
                    && list_n[0].id() == n.id()
                    {
                        // Last Node
                        //
                    ...
                    // //// CASE 1.2 //// //
                    else if list_m[0].id() == m.id() && m.id() != self.from_arena.size() - 1
                        && list_n[0].id() == self.to_arena.size() - 1
                        {
                        ...
                        // //// CASE 1.3 //// //
                        else if list_m[0].id() == self.from_arena.size() - 1
                            && list_n[0].id() == self.to_arena.size() - 1
                            {
                            ...
                            // //// CASE 1.4 //// //
                                ...
                                && list_n[0].id() == n.id()
                                && n.id() != self.to_arena.size() - 1
                                {
                                    // Check if parent of m,n is in edges
                                    for check in edges.clone() {
                                        let m_parent = self.from_arena[m].parent().unwrap();
                                        let n_parent = self.to_arena[n].parent().unwrap();
                ...
                // ///// CASE 2 //// //
                    ...
                    if list_m[0].id() == m.id() && m.id() != self.from_arena.size() - 1 {
                        // Partition
                        for n1 in &list_n {
                            for n2 in &list_n {
                                // Check if n1.parent == n2.parent are the same
                                if n1.id() != n2.id() {
                                    //                                    let n1_parent = self.from_arena[m].parent().unwrap();
                                    //                                    let n2_parent = self.to_arena[n].parent().unwrap();
                                }
                            }
                        }
                    }
    ...
    pub fn insert_node(&mut self, node_to_insert: NodeId<ToNodeId>) {
        self.list_node_inserted.push(node_to_insert);
    }
    /// add from_node to vector to be deleted
    pub fn delete_node(&mut self, node_to_delete: NodeId<FromNodeId>) {
        self.list_node_deleted.push(node_to_delete);
    }
    /// Print
    pub fn print(&self) {
        let size_from_node = NodeId::new(self.from_arena.size() - 1);
        println!("{:?}", size_from_node);
        let size_to_node = NodeId::<ToNodeId>::new(self.to_arena.size() - 1);
        let last_node_from_node = &self.from_arena[size_from_node];
        println!("The Last node From_NODE is --> {:?}", last_node_from_node);
        let last_node_to_node = &self.to_arena[size_to_node];
        println!("The Last node From_NODE is --> {:?}", last_node_to_node);
        println!("List Of edges already mapped to");
        let mut index = 0;
        for edges in &self.list_edges {
            println!(" -- {:?} -- ", index);

            println!(
                "TY        :   From node    -> {:?} || to node -> {:?}",
                self.from_arena[edges.from_node].ty, self.to_arena[edges.to_node].ty
            );
            println!(
                "Value     :   From node -> {:?} || to node -> {:?}",
                self.from_arena[edges.from_node].label, self.to_arena[edges.to_node].label
            );
            println!(
...
/// The Chawathe98Config2 matcher needs no configuration.
#[derive(Debug, Clone, PartialEq)]
pub struct Chawathe98Config2 {}

impl Default for Chawathe98Config2 {
    fn default() -> Chawathe98Config2 {
        Chawathe98Config2 {}
    }
}

impl Chawathe98Config2 {
    /// Create a new configuration object, with default values.
    pub fn new() -> Chawathe98Config2 {
        Default::default()
    }
}

impl<T: Clone + Debug + Eq + 'static> MatchTrees2<T> for Chawathe98Config2 {
    /// Perform matches
    fn match_trees(
        &self,
        ...
        let mut store = MappingStore2::new(base.clone(), diff.clone());
        ...
        //let last_node_from_node = base[size_fromNode];
        //let last_node_to_node = diff[size_toNode];
        ...
        //        store.print_preorder_traversal(base_pre.clone(),diff_pre.clone());
        //        println!("{:?}",diff_pre.clone());
        ...
        let get_all_edges = store.list_edges.clone();
        ...
        for edge in get_all_edges.clone() {
            if edge.edge_type == EdgeType::MOVE {
                ...
                let m_check = edge.from_node.is_leaf(&store.from_arena);
                let n_check = edge.to_node.is_leaf(&store.to_arena);
        ...
        //        println!("all leaves move => {:?}",all_leaves_move);
        for edge in all_leaves_move.clone() {
            let e1_parent = store.from_arena[edge.0].parent().unwrap(); //edge.0.parent;
            let e1_siblings = e1_parent
                .children(&store.from_arena)
                .collect::<Vec<NodeId<FromNodeId>>>();
            let e1_to_node_parent = store.to_arena[edge.1].parent().unwrap(); // edge to node parent
        ...
        for edge in all_leaves_parent.clone() {
            ...
            loop {
                if all_edges_move.contains(&(edge_change.0, edge_change.1, EdgeType::MOVE)) {
                    ...
                    let parent_m = store.from_arena[edge_change.0].parent().unwrap();
                    let parent_n = store.to_arena[edge_change.1].parent().unwrap();
                    if !parent_m.is_root(&store.from_arena) && !parent_n.is_root(&store.to_arena) {
    ...
    fn describe(&self) -> String {
        let desc = "This matcher performs matching operation from the paper Chawathe98.";
        String::from(desc)
    }

    /// Prune Edges
    fn prune_edges(&self, input: MappingStore2<T>) -> (Vec<Edge>, Vec<Edge>) {
        // Loop Though all the edges
        let ct1 = cmp::max(input.all_edge_cost.cost_move, input.all_edge_cost.cost_copy);

        let ct = cmp::max(ct1, input.all_edge_cost.cost_glue); // ct the max value of Move , copy and glue
        let all_edges = input.list_edges.clone();
        ...
        for e1_update in remaining_edges.clone() {
            if e1_update.edge_type == EdgeType::UPDATE {
                let m = e1_update
                    .from_node
                    .descendants(&input.from_arena)
                    .collect::<Vec<NodeId<FromNodeId>>>()
                    .len();
                let n = e1_update
                    .to_node
                    .descendants(&input.to_arena)
                    .collect::<Vec<NodeId<ToNodeId>>>()
                    .len();
        ...
        for e1_root in remaining_edges.clone() {
            // Check if its root
            if e1_root.from_node.is_root(&input.from_arena) {
            ...
            } else if e1_root.to_node.is_root(&input.to_arena) {
...
fn eq<T: Clone + Debug + Eq>(
    n1: &NodeId<FromNodeId>,
...
/// Test that two nodes have the same label.
fn eq_label<T: Clone + Debug + Eq>(
    n1: &NodeId<FromNodeId>,
    arena1: &Arena<T, FromNodeId>,
    n2: &NodeId<ToNodeId>,
    arena2: &Arena<T, ToNodeId>,
...
pub trait MatchTrees2<T: Clone + Debug> {
    /// Match two trees and return a store of mappings between them.
    fn match_trees(&self, base: Arena<T, FromNodeId>, diff: Arena<T, ToNodeId>)
                   -> MappingStore2<T>;
...
mod tests {
    ...
    pub fn test_chawathe_matching_1() -> () {
        ...
        let matching_config = Chawathe98Config2::new();
        let matcher = matching_config.match_trees(tree1.clone(), tree2.clone());
        let mut checker_edges_induced = matcher.list_edges.clone();
        ...
        //      Assert END
        // Visual Test
        //        let fileLocation = "./test_AS_1.dot";

        //        write_dotfile_to_disk(fileLocation, &matcher);
    }
    ...
    pub fn test_chawathe_matching_2() -> () {
        ...
        // ASSERT END
        // Visual Test
        //        let fileLocation = "./test_AS_2.dot";
        //        write_dotfile_to_disk(fileLocation, &matcher);
    }
    ...
    pub fn test_chawathe_matching_3() -> () {
        ...
        //
        //    println!("descendants1");
        let descendants1 = NodeId::new(0)
            .descendants(&tree1)
            .collect::<Vec<NodeId<FromNodeId>>>();
        let descendants2 = NodeId::new(0)
            .descendants(&arena)
            .collect::<Vec<NodeId<ToNodeId>>>();
        ...
        // Assert END
        // Visual Test
        //        let fileLocation = "./test_AS_3.dot";
        //        write_dotfile_to_disk(fileLocation, &matcher);
    }
    ...
    */
        ...
        // Visual Check
        //        write_dotfile_to_disk(file_location, &matcher);
    }
    ...
    pub fn test_chawathe_matching_6() -> () {
        ...
        let length_induced_edges = matcher.list_edges.clone().len();
        //
        let edges_pruned = pruning.0.clone();
        new_matcher_pruning.list_edges = edges_pruned.clone();
        // File Location Pruning
        // File Induced Graph
        //        let file_location_pruning = "./test_AS_6_prune.dot";
        //        write_dotfile_to_disk(file_location_pruning, &new_matcher_pruning);
        // Assert Test
        let mut checker_edges_pruned = matcher.list_edges.clone();
        // Assert Test
        ...
        // End of Pruning
        let file_location = "./test_AS_6.dot";
